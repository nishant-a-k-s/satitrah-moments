...
BEGIN
  ...
  INTO user_rec, stored_hash, attempt_count, is_locked
  FROM public.users u
  ...
  IF is_locked THEN
    RETURN QUERY SELECT user_rec.id, false, user_rec.auth_user_id;
    RETURN;
  END IF;
  ...
  IF stored_hash = public.hash_mpin(mpin_plain) THEN
    ...
    RETURN QUERY SELECT user_rec.id, true, user_rec.auth_user_id;
  ELSE
    -- Increment attempts
    UPDATE public.user_security 
